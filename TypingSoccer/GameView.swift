...
struct MenuView: View {
    ...
    var body: some View {
        ...
        .padding(40)
        .blur(radius: showExitConfirm ? 6 : 0)
        .disabled(showExitConfirm)
        if showExitConfirm {
            ExitConfirmOverlay(
                onCancel: {
                    Audio.button()
                    withAnimation(.easeOut(duration: 0.18)) { showExitConfirm = false }
                },
                onConfirm: {
...
/// Custom-styled confirmation card shown over a dimmed/blurred menu,
/// matching the game's yellow/monospaced visual language instead of a
/// plain system alert.
    ...
    var body: some View {
        ZStack {
            Color.black.opacity(0.55)
                .ignoresSafeArea()
                .onTapGesture { onCancel() }   // click outside to dismiss
            ...
            ZStack(alignment: .topLeading) {
                ...
                if coordinator.scene?.mode == .multiplayer {
                    HStack {
                        Spacer()
                        Text(
                            coordinator.peerConnected
                            ? L("lobby.connected")
                            : L("lobby.connectionLost")
                        )
                        .font(.system(size: 12, design: .monospaced))
    ...
    var body: some View {
        VStack(spacing: 10) {
            ...
            LazyVGrid(columns: columns, spacing: 14) {
                ...
                stat(L("results.mistakes"), "\(stats.mistakes)")
                stat(L("results.fastest"), fastest)
                stat(L("results.shots"), "\(stats.shotsScored)/\(stats.shotsTaken)")
                stat(L("results.saves"), "\(stats.savesMade)/\(stats.savesFaced)")
                stat(L("results.bestCombo"), "\(stats.bestCombo)")
            }
        }
        .padding(18)
        .frame(maxWidth: 460)
